...
def update_rover(Rover, data):
      ...
      # Or just update elapsed time
            ...
            tot_time = time_now - Rover.start_time
      ...
      if Rover.mode == 'forward' or Rover.mode == 'mission':
            ...
            priming = False
            if Rover.pos_q == None:
                  Rover.pos_q = Rover.pos
                  priming = True
            if Rover.time_q == None:
                  Rover.time_q = time_now
                  priming = True
            if priming == False:
                  dx = Rover.pos[0] - Rover.pos_q[0]
                  dy = Rover.pos[1] - Rover.pos_q[1]
                  dist = np.sqrt(dx**2 + dy**2)
                  delta_t = time_now - Rover.time_q
                  
                  print('Progress check: distance {} delta_t {}'.format(dist,delta_t))
